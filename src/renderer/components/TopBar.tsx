...
export function TopBar({ gatewayUrl, connectionStatus, theme, onConnect, onToggleTheme, onOpenPalette }: TopBarProps) {
      ...
      <div className="flex items-center gap-2 flex-1 max-w-md no-drag">
        <div className="relative flex-1">
          <input
            ...
            className="w-full h-8 pl-3 pr-3 text-xs bg-reef-bg border border-reef-border rounded-lg text-reef-text-bright placeholder-reef-text-dim focus:border-reef-accent focus:ring-1 focus:ring-reef-accent/30 focus:outline-none transition-all duration-150 font-mono"
          />
        </div>
        <button
          onClick={() => onConnect(url)}
          className={`h-8 px-3 text-xs rounded-lg font-medium flex items-center gap-1.5 transition-all duration-150 ring-1 ${
